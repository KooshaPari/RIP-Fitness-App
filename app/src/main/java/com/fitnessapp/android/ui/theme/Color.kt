package com.fitnessapp.android.ui.theme

import androidx.compose.ui.graphics.Color

/**
 * Material 3 color definitions for FitnessApp
 * Generated using Material Theme Builder
 * Provides consistent branding colors across the app
 */

// Primary color palette - Fitness theme (energetic blue-green)
val md_theme_light_primary = Color(0xFF006B5A)
val md_theme_light_onPrimary = Color(0xFFFFFFFF)
val md_theme_light_primaryContainer = Color(0xFF7BF7DD)
val md_theme_light_onPrimaryContainer = Color(0xFF002019)

val md_theme_dark_primary = Color(0xFF5FDBC1)
val md_theme_dark_onPrimary = Color(0xFF00382D)
val md_theme_dark_primaryContainer = Color(0xFF005143)
val md_theme_dark_onPrimaryContainer = Color(0xFF7BF7DD)

// Secondary color palette - Complementary orange for accents
val md_theme_light_secondary = Color(0xFF4B635B)
val md_theme_light_onSecondary = Color(0xFFFFFFFF)
val md_theme_light_secondaryContainer = Color(0xFFCDE9DE)
val md_theme_light_onSecondaryContainer = Color(0xFF072019)

val md_theme_dark_secondary = Color(0xFFB1CCC2)
val md_theme_dark_onSecondary = Color(0xFF1D352E)
val md_theme_dark_secondaryContainer = Color(0xFF344C44)
val md_theme_dark_onSecondaryContainer = Color(0xFFCDE9DE)

// Tertiary color palette - Warm orange for workout/energy elements
val md_theme_light_tertiary = Color(0xFF3E6472)
val md_theme_light_onTertiary = Color(0xFFFFFFFF)
val md_theme_light_tertiaryContainer = Color(0xFFC2E9F9)
val md_theme_light_onTertiaryContainer = Color(0xFF001F29)

val md_theme_dark_tertiary = Color(0xFFA6CDDD)
val md_theme_dark_onTertiary = Color(0xFF0B343F)
val md_theme_dark_tertiaryContainer = Color(0xFF254C58)
val md_theme_dark_onTertiaryContainer = Color(0xFFC2E9F9)

// Error colors
val md_theme_light_error = Color(0xFFBA1A1A)
val md_theme_light_errorContainer = Color(0xFFFFDAD6)
val md_theme_light_onError = Color(0xFFFFFFFF)
val md_theme_light_onErrorContainer = Color(0xFF410002)

val md_theme_dark_error = Color(0xFFFFB4AB)
val md_theme_dark_errorContainer = Color(0xFF93000A)
val md_theme_dark_onError = Color(0xFF690005)
val md_theme_dark_onErrorContainer = Color(0xFFFFDAD6)

// Surface colors
val md_theme_light_background = Color(0xFFFAFDFA)
val md_theme_light_onBackground = Color(0xFF191C1A)
val md_theme_light_surface = Color(0xFFFAFDFA)
val md_theme_light_onSurface = Color(0xFF191C1A)

val md_theme_dark_background = Color(0xFF0F1511)
val md_theme_dark_onBackground = Color(0xFFE0E3E0)
val md_theme_dark_surface = Color(0xFF0F1511)
val md_theme_dark_onSurface = Color(0xFFE0E3E0)

// Surface variant colors
val md_theme_light_surfaceVariant = Color(0xFFDBE5E1)
val md_theme_light_onSurfaceVariant = Color(0xFF404944)

val md_theme_dark_surfaceVariant = Color(0xFF404944)
val md_theme_dark_onSurfaceVariant = Color(0xFFC0C9C5)

// Outline colors
val md_theme_light_outline = Color(0xFF707974)
val md_theme_light_outlineVariant = Color(0xFFBFC9C5)

val md_theme_dark_outline = Color(0xFF8A938E)
val md_theme_dark_outlineVariant = Color(0xFF404944)

// Inverse colors
val md_theme_light_inverseSurface = Color(0xFF2E312F)
val md_theme_light_inverseOnSurface = Color(0xFFEFF1EE)
val md_theme_light_inversePrimary = Color(0xFF5FDBC1)

val md_theme_dark_inverseSurface = Color(0xFFE0E3E0)
val md_theme_dark_inverseOnSurface = Color(0xFF2E312F)
val md_theme_dark_inversePrimary = Color(0xFF006B5A)

// Surface tint and scrim
val md_theme_light_surfaceTint = Color(0xFF006B5A)
val md_theme_light_scrim = Color(0xFF000000)

val md_theme_dark_surfaceTint = Color(0xFF5FDBC1)
val md_theme_dark_scrim = Color(0xFF000000)

// Custom colors for fitness-specific UI elements
val SuccessGreen = Color(0xFF00C851)
val WarningOrange = Color(0xFFFF8800)
val InfoBlue = Color(0xFF0099CC)
val CalorieRed = Color(0xFFFF4444)
val ProteinBlue = Color(0xFF2196F3)
val CarbOrange = Color(0xFFFF9800)
val FatPurple = Color(0xFF9C27B0)